...
public class ReadMusicItemRequest {
     ...
     */
    public static ReadMusicItemRequest checkRequest(
            final FormItemList request,
            final ReadResponse response) {
        final String musicItemIdentifier =
                checkMusicItemIdentifier(request, response);
        if (musicItemIdentifier != null) {
            final MusicItemVersion musicItemVersion =
                    checkMusicItemVersion(request, response);
    ...
    protected static MusicItemVersion checkMusicItemVersion(
            ...
            final ReadResponse response) {
        try {
            final String musicItemVersionName =
                    formItemList
                            .getField(ProtocolConstants.Parameter.Read.MUSIC_ITEM_VERSION);
            final MusicItemVersion musicItemVersion =
                    MusicItemVersion.getMusicItemVersion(musicItemVersionName);
